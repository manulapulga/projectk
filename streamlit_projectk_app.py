...
# =============================
def main():
    """Main app with error catching for initialization issues."""
    # Clear any existing debug messages
    debug_container = st.container()
    
    with debug_container:
        try:
            # First attempt to set page config
            st.set_page_config(
            ...
            with col1:
                st.markdown("**Check file paths:**")
                st.code(f"""
                # Current working directory: {os.getcwd()}
                # QUESTION_DATA_FOLDER exists: {os.path.exists(QUESTION_DATA_FOLDER)}
                # LOGIN_FILE_PATH exists: {os.path.exists(LOGIN_FILE_PATH)}
                """)
            
            with col2:
                st.markdown("**Check dependencies:**")
                st.code("""
                # Run in terminal:
                pip install streamlit pandas numpy openpyxl psutil firebase-admin
                """)
