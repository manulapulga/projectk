...
# LitmusQ Color Theme
LITMUSQ_THEME = {
    "primary": "#21918e",      # Dark blue
    "secondary": "#dc56e8",    # Red
...
def delete_user(username):
    """Delete user and ALL their progress data from Firestore."""
    try:
        if db is None:
            st.error("Firebase not initialized.")
            return False

        # Do not delete admin/editor users
        admin_credentials = load_admin_credentials()
        editor_credentials = load_editor_credentials()
        if username in admin_credentials or username in editor_credentials:
        ...
        try:
            uref = db.collection("users").document(username)
            if uref.get().exists:
                uref.delete()
        except Exception as e:
            st.warning(f"Could not delete user doc: {e}")

        # 2) Delete ALL user progress documents
        progress_ids = [
            username,
            f"user_{username}",  # explicit — avoids issues
            get_user_progress_doc_id(username)  # redundant but safe
        ]

        # Remove duplicates
        progress_ids = list(set(progress_ids))
        ...
        for doc_id in progress_ids:
            ...
            if doc_ref.get().exists:
                ...
                try:
                    tests_ref = doc_ref.collection("tests")
                    for tdoc in tests_ref.stream():
                        tdoc.reference.delete()
                except Exception as e:
                    st.warning(f"Could not delete tests in {doc_id}: {e}")

                # Delete meta subcollection
                try:
                    meta_ref = doc_ref.collection("meta")
                    for mdoc in meta_ref.stream():
                        mdoc.reference.delete()
                except Exception as e:
                    st.warning(f"Could not delete meta in {doc_id}: {e}")

                # Delete main progress doc
                try:
                    doc_ref.delete()
                except Exception as e:
                    st.warning(f"Could not delete progress doc {doc_id}: {e}")
...
def initialize_user_progress(username):
    """Create lightweight user profile document."""
    try:
...
def save_test_result(username, test_entry):
    """Save individual test attempt in /tests/ subcollection."""
    try:
        tests_ref = db.collection("user_progress")\
                      .document(username)\
                      .collection("tests")\
                      .document(test_entry["test_id"])

        tests_ref.set(test_entry)
        return True
    except Exception as e:
...
def update_user_profile(username, updated_data):
    ...
    try:
        ...
        profile_ref.set(updated_data, merge=True)
        return True
    except Exception as e:
...
def load_test_history(username):
    """Load all test attempts (separate documents)."""
    try:
        tests_ref = db.collection("user_progress")\
                      .document(username)\
                      .collection("tests")
...
def clear_user_progress(username):
    """Delete ALL user test documents + reset profile."""
    try:
        # Delete all test docs
        tests_ref = db.collection("user_progress")\
                      .document(username)\
                      .collection("tests")
...
def update_user_progress(test_results):
    ...
    # --- Update small profile stats ---
    tests_taken = int(profile.get("tests_taken", 0)) + 1
    total_score = float(profile.get("total_score", 0)) + float(test_results["Marks Obtained"])
    ...
    # --- Build heavy test entry (stored separately) ---
...
def show_enhanced_question_interface():
    ...
    if st.session_state.end_time and not st.session_state.submitted:
        ...
        <div id="timer" style="
            ...
            margin: 0;
            padding: 0;
            line-height: 1;  /* 🔥 reduces vertical height */
            display: inline-block; /* 🔥 prevent extra block height */
        "></div>
